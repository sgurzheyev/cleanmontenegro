-- Escrow payout split for approve decision:
-- Cleaner: 90%
-- Scout (creator): 5.1%
-- Platform fee: remaining 4.9% stays with the platform (not credited to any user wallet)
...
begin
  ...
  if lower(coalesce(p_decision, '')) = 'approve' then
    ...
    -- Move full escrow out of creator frozen_balance
    update public.profiles
      set frozen_balance = coalesce(frozen_balance, 0) - v_funding
    where id = v_mission.creator_id;

    -- Credit split rewards (platform keeps remaining 4.9%)
    update public.profiles
    ...
    -- 3 Strikes logic: do not require/validate funding on reject
    update public.missions
