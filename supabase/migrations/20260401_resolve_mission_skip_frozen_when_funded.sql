-- When mission current_funding already covers the payout base (v_funding), skip debiting creator
-- frozen_balance — funds are already in the mission pool (e.g. crowdfunding) and the
-- "Insufficient creator frozen_balance" check was incorrectly blocking release.
...
begin
  ...
  if lower(coalesce(p_decision, '')) = 'approve' then
    ...
    -- Mission pool already holds the payout base: do not require matching creator frozen escrow.
    ...
    -- Paid out: mission lifecycle complete (review → client approve → completed).
